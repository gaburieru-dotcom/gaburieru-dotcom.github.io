...
for f in files:
    if f == "index.html" or f == "vertical-jump.html":
        # index.html already has the apps list.
        # vertical-jump.html redirects or is the landing page for the old jump app. formx-guide.html is the new one. Wait, vertical-jump.html is an old page, maybe skip it or modify it. Let's just modify all guides.
        if f == "index.html":
            continue
    ...
    if "global-app-intro" in content:
        continue
        
    # Inject before </main>
    # If the file has <nav class="reading-next">, inject before it
    if '<nav class="reading-next"' in content:
        content = content.replace('<nav class="reading-next"', app_block + '\n  <nav class="reading-next"')
    elif '</main>' in content:
        content = content.replace('</main>', app_block + '\n</main>')
    else:
        print(f"Skipping {f}, no </main> found.")
        continue
